...
export default {
  ...
  methods: {
    async loadTasks() {
      const {
        data: { data },
      } = await this.$axios.get(this.getTasksIndexUrl());
    ...
    getProjectCategoriesBreadcrumbs() {
      ...
      const result = [];
      this.parentCategories.map((item) =>
        result.push({
          label: item.title,
    ...
    async loadChildrenCategories() {
      const {
        data: { data: category },
      } = await this.$axios.get(`api/project_categories/${this.id}/with/parents`);
    ...
    async loadCategories() {
      const {
        data: { data },
      } = await this.$axios.get(`api/project_categories/children/${this.id}?all`);
      if (this.id) {
    ...
    async deleteCategory(category) {
      await this.$axios.delete(`api/project_categories/${category.id}`);
      this.loadCategories();
    },
