  ...
  <div class="w-full items-center justify-center">
    <div v-for="(task, i) in tasks" :key="i" class="w-full px-2 mt-2">
      <div
        ...
        :class="
          task.start_time ? `border-solid border-l-8 border-green-600` : ``
        "
      >
        <div class="w-full">
          <div
...
export default {
  ...
  methods: {
    async stopCountdown(task, dotId) {
      ...
      await this.$axios.delete(`api/tasks/${task.id}/countdown`);
    ...
    getShowButtons(task) {
      return {
          ...
          (this.status !== "done" && !this.useTaskStatusForButtons) ||
          (this.useTaskStatusForButtons && task.status !== "done"),
        activate:
          this.status === "done" ||
          (this.status === "hidden" && !this.useTaskStatusForButtons) ||
          (this.useTaskStatusForButtons &&
            (task.status === "done" || task.status === "hidden")),
        hide:
          (this.status !== "hidden" &&
            this.status !== "done" &&
            !this.useTaskStatusForButtons) ||
    ...
    async startCountdown(task, dotId) {
      this.isLoadingActions[dotId] = true;
      await this.$axios.post(`api/tasks/${task.id}/countdown`);
      await this.loadTasks();
      this.isLoadingActions[dotId] = false;
    },
    async loadTasks() {
    ...
    getActions(task) {
      ...
      actions = this.addActionItem(
        actions,
        this.getActionItem(task, "active", "Activate"),
        this.getShowButtons(task).activate
      );
      actions = this.addActionItem(
        actions,
        this.getActionItem(task, "hidden", "Hide"),
        this.getShowButtons(task).hide
      );
      actions = this.addActionItem(
        actions,
        this.getActionItem(task, "done", "Done"),
        this.getShowButtons(task).done
      );
    ...
    async updateStatus(task, status, dotId) {
      try {
        this.isLoadingActions[dotId] = true;
        await this.$axios.put(`api//tasks/${task.id}/${status}`);
        await this.loadTasks();
        this.isLoadingActions[dotId] = false;
      } catch (e) {
        this.isLoadingActions[dotId] = false;
