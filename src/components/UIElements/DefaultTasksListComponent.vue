  ...
  <table class="w-full shadow-lg rounded mt-5">
    <tbody :class="`${$color('blocks')}`">
      <tr
        v-for="task in tasks"
        :key="task.id"
        :class="`accordion border-b ${$color('borderMain')} hover:${$color(
          'blocksHover'
        )}`"
        @click="panel = true"
      >
        <td class="flex inline-flex items-center">
          <span class="py-3 w-40">
            ...
            <p
              :class="`md:hidden text-xs ${$color('textMain')}-600 font-medium`"
            >
              {{ capitalize(task.status) }}
            </p>
            <p
              v-if="task.category"
              :class="`pl-2 hidden md:table-cell text-xs text-center w-full ${$color(
                'textMain'
              )}-500 font-medium`"
            >
              <router-link
        ...
        <td class="hidden md:table-cell">
          ...
          <p v-else :class="`text-sm ${$color('textMain')}-700 font-medium`">
            -
          </p>
        </td>
        <td>
          ...
          <div class="hidden lg:block">
            ...
            <Button color="blue" @click="$router.push(`/tasks/${task.id}/edit`)"
              >Open task</Button
            >
            <Button
